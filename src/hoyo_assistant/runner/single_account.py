...
async def run_once(
    ...
    # 每个任务块单独捕获异常，避免单次网络/超时错误导致整个进程退出
    raise_stoken = False
    try:
        ...
        log.error(t("mihoyobbs.task_error", error=e))
        return_data.append(t("mihoyobbs.task_error", error=e))

    try:
        cn_result = await run_cn_signin_tasks()
    ...
    try:
        ...
        log.error(t("web_activity.task_error", error=e))
        return_data.append(t("web_activity.task_error", error=e))
