...
class Mihoyobbs:
    def __init__(self):
        ...
        self.bbs_list = [
            MIHOYOBBS_POST_TYPES.get(i)
            for i in self.bbs_config["checkin_list"]
            if MIHOYOBBS_POST_TYPES.get(i) is not None
        ]
        self.headers = {
        ...
        self.task_do = {"sign": False, "read": False, "read_num": 3, "like": False, "like_num": 5, "share": False}
        self.postsList = []
    ...
    async def get_pass_challenge(self):
    ...
    # 获取任务列表，用来判断做了哪些任务
    async def get_tasks_list(self, update=False):
    ...
    async def signing(self):
        ...
        for forum in self.bbs_list:
            ...
            for _retry_count in range(2):
                ...
                header["DS"] = tools.get_ds2("", post_data)  # DS2 might be CPU bound but okay
                ...
                if data["retcode"] == 1034:
                    ...
                    log.info(t("mihoyobbs.sign_success", forum=forum["name"], message=data.get("message", "OK")))
                    await wait()
                    break
    ...
    # 看帖子
    async def read_posts(self, post_info):
        req = await http.get(url=API_BBS_POST_DETAIL, params={"post_id": post_info[0]}, headers=self.headers)
        log.debug(await req.text())
        data = await req.json()
        if data["message"] == "OK":
    ...
    # 点赞
    async def like_posts(self, post_info, captcha_try: bool = False):
        ...
        if captcha_try:
            ...
            if challenge is not None:
                ...
                # 验证码没通过
                await wait()
        req = await http.post(url=API_BBS_LIKE, headers=header, json={"post_id": post_info[0], "is_cancel": False})
        log.debug(await req.text())
        data = await req.json()
        if data["message"] == "OK":
    ...
    # 取消点赞
    async def cancel_like_post(self, post_info):
        req = await http.post(url=API_BBS_LIKE, headers=self.headers, json={"post_id": post_info[0], "is_cancel": True})
    ...
    # 分享操作
    async def share_post(self, post_info):
        for i in range(3):
            req = await http.get(
                url=API_BBS_SHARE, params={"entity_id": post_info[0], "entity_type": 1}, headers=self.headers
            )
            log.debug(await req.text())
            data = await req.json()
            if data["message"] == "OK":
    ...
    async def run_task(self):
        ...
        if self.task_do["sign"] and self.task_do["read"] and self.task_do["like"] and self.task_do["share"]:
            return_data += t("mihoyobbs.summary_done", got=self.today_have_get_coins, total=self.have_coins)
            log.info(t("mihoyobbs.summary_done", got=self.today_have_get_coins, total=self.have_coins))
            return return_data
        i = 0
