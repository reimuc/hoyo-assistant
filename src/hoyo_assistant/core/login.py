...
from . import CookieError, StokenError, config, http, log, setting, t
...
async def login() -> None:
    ...
    if require_cookie_token():
        ...
        req = await http.get(
            API_GET_TOKEN_BY_STOKEN,
            params={"uid": uid, "stoken": config["account"]["stoken"]},
        )
        data = await req.json()
        if data["retcode"] != 0:
...
def get_login_ticket() -> str | None:
    ticket_match = re.search(
        r"login_ticket=(.*?)(?:;|$)", str(config["account"]["cookie"])
    )
...
async def get_cookie_token_by_stoken() -> str:
    if config["account"]["stoken"] == "" and config["account"]["stuid"] == "":
...
async def update_cookie_token() -> bool:
    ...
    req = await http.get(
        API_GET_TOKEN_BY_STOKEN,
        params={
            "uid": config["account"]["stuid"],
            "stoken": config["account"]["stoken"],
        },
    )
    data = await req.json()
    if data["retcode"] != 0:
        ...
        await setting.clear_stoken()
        return False
    config["account"]["cookie"] += f" cookie_token={data['data']['token']['token']};"
    await setting.save_config()
    return True
...
def require_mid() -> bool:
    ...
    :return: 是否需要mid
    """
    return str(config["account"]["stoken"]).startswith("v2_")
...
def get_stoken_cookie() -> str:
    ...
    """
    cookie = f"stuid={config['account']['stuid']};stoken={config['account']['stoken']}"
    if require_mid():
        if config["account"]["mid"]:
            cookie += f";mid={config['account']['mid']}"
        else:
...
def require_stoken() -> bool:
    ...
    :return: 是否需要 stoken
    """
    return str(config["account"]["stoken"]).startswith("v1_")
